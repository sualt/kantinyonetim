...
router.post('/', (req, res) => {
  ...
  const productRow = db.prepare('SELECT price FROM products WHERE name = ?').get(product.trim());
  const price = productRow ? Number(productRow.price) : 0;
...
router.patch('/:id/payment', (req, res) => {
  ...
  const db = getDb();
  const sale = db.prepare('SELECT s.id, s.person_id, s.product, s.quantity, s.paid FROM sales s WHERE s.id = ?').get(id);
  if (!sale) {
    return res.status(404).json({ error: 'İşlem bulunamadı' });
  }

  const productRow = db.prepare('SELECT price FROM products WHERE name = ?').get(sale.product);
  const price = productRow ? Number(productRow.price) : 0;
  const totalCost = price * Number(sale.quantity);
  ...
  const updatedSale = db.prepare('SELECT s.id, s.person_id, p.name AS person_name, s.product, s.quantity, s.paid, s.created_at FROM sales s JOIN persons p ON p.id = s.person_id WHERE s.id = ?').get(id);
